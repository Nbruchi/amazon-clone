...
const ProductCard = ({ product, className = "", style }: ProductCardProps) => {
    ...
    const renderStars = (rating: number) => {
        ...
        for (let i = 0; i < fullStars; i++) {
            stars.push(
                <span key={i} className="text-amazon-yellow">
                    ★
                </span>
            );
        }
        ...
        if (hasHalfStar) {
            stars.push(
                <span key="half" className="text-amazon-yellow">
                    ☆
                </span>
            );
        }
    ...
    return (
        <div
            className={`bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden group hover-lift ${className}`}
            style={style}
        >
            {/* Product Image */}
            ...
            <div className="p-4">
                <div className="mb-2">
                    <span className="text-xs text-amazon-purple font-medium">
                        {product.category}
                    </span>
                </div>
